...
A comprehensive suite of sample and tutorial application are provided in the `/examples` subdirectory. For new users, it is best to review the tutorial series of apps which are designed to progressively introduce API features.
  ...
  * [Projection APIs](./doc/projection.md) - A guide and coordinate systems, calibration information, and projection
  * [Camera Spec Sheet](./doc/camera_specs.md) - A brief overview of R200, F200 and SR300
  * [Developer Notes](./doc/dev_log.md) - Serveral informal notes gathered during internal releases
  * [Stream Formats](./doc/supported_video_formats.pdf) - A list of available stream resoluations and pixel formats provied by the supported devices.
  * [Branching and Relases](./doc/branching.md) - Overview of live branches and major release
...
4. Majority of hardware-specific functionality of individual camera generations (UVC XU controls).
5. Multi-camera capture acorss heterogeneous camera architectures (e.g. mix R200 and F200 in same application)
6. Motion-tracking sensors acquistion (ZR300 only)
...
5. [RealSense ZR300](https://newsroom.intel.com/chip-shots/intel-announces-tools-realsense-techology-development/)
...
The library is written in standards-conforming C++11 and relies only on the C80 ABI for its public interface. It is developed and tested on the following platforms:
...
Developer kits containing the necessary hardware to use the library are available for purchase at [this link](http://click.intel.com/realsense.html). In addition, serveral consumer tablets and loptops with integrated cameras may also function, such as the [HP Spectre x2 with R200](https://store.hp.com/us/en/ContentView?storeId=10151&langId=-1&catalogId=10051&eSpotName=new-detachable).

Developer kits **require** USB 3.0. RealSense™ cameras do not provide backwards compatibility with USB 2.0. Not all USB host chipsets are compatible with this library, although it has been validated with recent generations of the Intel Host Controller shipset. An exhaustive list of incompatible hardware is ot presently provided. On x86, a Haswell or newer architecture is recommended.
...
For small-form factor usages, this library has been demonstrated to work on the following boards:
  ...
  * [MinnowBoard Max](http://minowboard.org)
...
you may not use this project expect in compliance with the License.
...
See the License for the specific languare governing permissions and
